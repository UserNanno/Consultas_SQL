...
M_SOLICITUDINDIVIDUO AS (
  SELECT
    ...
    (CASE
    	WHEN DESTIPROLSOLICITUD = 'TITULAR' THEN MTOINGRESADODIGITADO
    	ELSE NULL
    END) AS MTOIGRESOBRUTOTITULAR,
    (CASE
    	WHEN DESTIPROLSOLICITUD = 'CONYUGE' THEN MTOINGRESADODIGITADO
    	ELSE NULL
    END) AS MTOIGRESOBRUCONYUGE,
    (CASE
    	WHEN DESTIPROLSOLICITUD = 'TITULAR' THEN MTOOTROINGRESODIGITADO
    	ELSE NULL
    END) AS MTOOTROIGRESOBRUTOTITULAR,
    (CASE
    	WHEN DESTIPROLSOLICITUD = 'CONYUGE' THEN MTOOTROINGRESODIGITADO
    	ELSE NULL
    END) AS MTOOTROIGRESOBRUTOCONYUGE, 
    (CASE
    ...
    MTOCEMANALISTACREDITO AS CEMANALISTA,
    (CASE
    ...
    (CASE
    	WHEN DESTIPROLSOLICITUD = 'TITULAR' THEN DESTIPESTADOCIVIL
    	ELSE NULL
    END) AS ESTADOCIVILTITULAR,
    (CASE
    	WHEN NUMPUNTAJEPRECALIF <= 0 THEN 'SIN DATA'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 1 AND 300 THEN 'A. 1-300'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 301 AND 330 THEN 'B. 301-330'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 331 AND 360 THEN 'C. 331-360'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 361 AND 390 THEN 'D. 361-390'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 391 AND 420 THEN 'E. 391-420'
    	WHEN NUMPUNTAJEPRECALIF BETWEEN 421 AND 450 THEN 'F. 421-450'
    	ELSE NULL
    END) AS RANGOSCORETITULAR
  FROM CATALOG_LHCL_PROD_BCP.BCP_UDV_INT_VF.M_SOLICITUDINDIVIDUO
),
M_SOLICITUDPARTY AS (
  SELECT
    CODSOLICITUD,
  ...
  FROM CATALOG_LHCL_PROD_BCP.BCP_UDV_INT_VF.M_SOLICITUDPARTY
),
USERAPP AS (
  SELECT
    CODUSUARIOAPP,
    DESPOSICIONCOLABORADOR
  FROM CATALOG_LHCL_PROD_BCP.BCP_UDV_INT_VF.M_USUARIOAPLICATIVO
)
