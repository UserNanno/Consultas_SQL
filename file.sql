...
M_SOLICITUDTARJETACREDITO AS (
	SELECT
	    ...
	    (CASE
	      ...
	      WHEN INSTR(A.DESCAMPANIA, 'PRESTAMO TAR') > 0 OR INSTR(A.DESCAMPANIA, 'PTC') > 0 OR INSTR(A.DESCAMPANIA, 'EP') > 0
	        OR INSTR(A.DESCAMPANIA, 'BALANCE TRA') > 0 OR INSTR(A.DESCAMPANIA, 'TC+BT') > 0 OR INSTR(A.DESCAMPANIA, 'BT') > 0
	        OR INSTR(A.DESCAMPANIA, 'AMPLIACION') > 0 OR INSTR(A.DESCAMPANIA, 'UPGRADE') > 0 OR INSTR(A.DESCAMPANIA, 'UPG') > 0
	        OR INSTR(A.DESCAMPANIA, 'ADICIONAL') > 0
...
TP_BASE AS (
	...
	LEFT JOIN M_SOLICITUDINDIVIDUO_T B ON (A.CODSOLICITUD = B.CODSOLICITUD)
	LEFT JOIN M_SOLICITUDINDIVIDUO_C C ON (A.CODSOLICITUD = C.CODSOLICITUD)
	LEFT JOIN M_SOLICITUDPARTY_T    D ON (A.CODSOLICITUD = D.CODSOLICITUD)
	LEFT JOIN M_SOLICITUDPARTY_C    E ON (A.CODSOLICITUD = E.CODSOLICITUD)
