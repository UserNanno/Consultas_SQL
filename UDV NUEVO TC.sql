...
HM_LEADS_BDI AS (
	SELECT
		...
		FECINICIOVIGENCIA,
		date_format(FECINICIOVIGENCIA, 'yyyyMM') AS CODMESINICIOVIGENCIA,
		(CASE
	...
	WHERE CODPAUTARBM <> 41 AND TIPVENTA IN (6,110) AND A.DESCAMPANA NOT IN ('CREDITO PERSONAL, VENTA AMPLIACION PLAZO', 'VENTA SKIP')
),
BASE_LEADS AS (
	SELECT
		A.CODLOTEOFERTA,
...
--			WHEN A.TIPVENTA = 127 AND A.TIPOFERTA = 3 THEN 'LD MULTI' - FALTA TRAER TIPVENTA 127 DE BASE DE LEADS
	...
	WHERE 
		...
		AND CODMESINICIOVIGENCIA = date_trunc('MM', to_date(CAST(P.CODMES AS STRING), 'yyyyMM'))
)
