...
BASE_FLAGS AS (
  SELECT
    ...
    CASE WHEN m.DESTIPDECISIONRESULTADORBM IN ('Decline','Investigate') AND m.FLG_REINGRESO_1M=1 THEN 1 ELSE 0 END AS FLG_DENEG_REINGRESO_1M_NO_VENTA,
    CASE WHEN m.DESTIPDECISIONRESULTADORBM IN ('Decline','Investigate') AND m.FLG_REINGRESO_1M=0 THEN 1 ELSE 0 END AS FLG_DENEG_NOREINGRESO_NO_VENTA,
    m.FLG_REINGRESO_1M,
...
BASE_SBS AS (
  SELECT
    x.*,
    COALESCE(s1.MTODEUDASOL_SISTEMA,0) AS MTO_DEUDA_SBS_MES_1,
    ...
    CASE WHEN COALESCE(s2.MTODEUDASOL_SISTEMA,0) - COALESCE(s1.MTODEUDASOL_SISTEMA,0) > 0 THEN COALESCE(s2.MTODEUDASOL_SISTEMA,0) - COALESCE(s1.MTODEUDASOL_SISTEMA,0) ELSE 0 END AS MTO_AUMENTO_DEUDA_SBS,
...
FUNNEL_SBS AS (
  SELECT
    ...
    SUM(FLG_DENEG_REINGRESO_1M_NO_VENTA) AS N_DENEG_REING_NO_VTA,
    SUM(FLG_DENEG_NOREINGRESO_NO_VENTA) AS N_DENEG_NOREING_NO_VTA,
    SUM(FLG_DESEMBOLSADO) AS N_DESEMBOLSADAS,
    SUM(FLG_NODESEMBOLSADO) AS N_APROB_NO_DESEM,
    SUM(FLG_REINGRESO_1M) AS N_REINGRESOS,
    SUM(FLG_REINGRESO_1M_DESEMBOLSO) AS N_REING_DESEM,
    SUM(FLG_REINGRESO_1M_NO_DESEMBOLSO) AS N_REING_NO_DESEM,
    SUM(FLG_NOREINGRESO_NODESEMBOLSO) AS N_NOREING_NO_DESEM,
    SUM(MTO_TOTAL_DESEMBOLSADO) AS MTO_TOTAL_DESEMBOLSADO,
    SUM(MTO_TOTAL_ORIGINAL) AS MTO_TOTAL_ORIGINAL,
    SUM(MTO_DESEMBOLSADO_REINGRESO_1M) AS MTO_DESEMBOLSADO_REINGRESO_1M,
    SUM(FLG_AUMENTO_DEUDA_SBS) AS N_AUM_DEUDA_SBS,
    SUM(MTO_AUMENTO_DEUDA_SBS) AS MTO_AUM_DEUDA_SBS,
    SUM(FLG_AUMENTO_DEUDA_SBS_REGLA) AS N_AUM_DEUDA_SBS_REGLA,
    SUM(MTO_AUMENTO_DEUDA_SBS_REGLA) AS MTO_AUM_DEUDA_SBS_REGLA,
    SUM(CASE WHEN FLG_DENEG_REINGRESO_1M_NO_VENTA = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_DENEG_REING_NO_VTA_CON_AUM_SBS_REGLA,
    SUM(CASE WHEN FLG_DENEG_NOREINGRESO_NO_VENTA = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_DENEG_NOREING_NO_VTA_CON_AUM_SBS_REGLA,
    SUM(CASE WHEN FLG_NODESEMBOLSADO = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_APROB_NO_DESEM_CON_AUM_SBS_REGLA
  FROM BASE_SBS
  GROUP BY
    CODMESSOLICITUDEVALUACION,
    DESCANALVENTARBM,
