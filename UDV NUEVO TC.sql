%sql
CREATE TABLE IF NOT EXISTS catalog_lhcl_prod_bcp.bcp_edv_rbmbdn.T72496_SOLICITUDES_HIST (
  CODMES                INT,
...
LOCATION 'abfss://bcp-edv-rbmbdn@adlscu1lhclbackp05.dfs.core.windows.net/T72496/SOLICITUDES_HIST'
;
...
BEGIN
  ...
  bucle: LOOP
    ...
    IF MOD(v_mes, 100) = 12 THEN
      SET v_mes_1 = (DIV(v_mes, 100) + 1) * 100 + 1;   -- YYYY12 -> (YYYY+1)01
    ELSE
      SET v_mes_1 = v_mes + 1;
    END IF;
...
LEADS_RAW AS (
  SELECT
    ...
    ROUND(A.MTOFINALOFERTADOSOL,1)  AS MTO_OFERTA_LEAD,
...
SOLICITUDES_LEAD AS (
  SELECT
    ...
    A.MTO_SOLICITADO,
    A.MTO_APROBADO,
    COALESCE(B.MTO_OFERTA_LEAD, C.MTO_OFERTA_LEAD) AS MTO_OFERTA_LEAD
  FROM RBM_VTA_SOL A
...
BASE AS (
  SELECT DISTINCT
    ...
    COALESCE(A.MTO_OFERTA_LEAD,0)             AS MTO_OFERTA_LEAD
