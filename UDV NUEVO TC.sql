...
VENTA AS (
  SELECT
    ...
    CASE
      ...
      WHEN SUBSTR(TRIM(CODSOLICITUD),1,2)='JB' THEN 'BANCA M VIL'
...
SBS_DEUDA AS (
  ...
  WHERE CAST(date_format(FECDIA,'yyyyMM') AS INT) BETWEEN 202412 AND 202512
    AND NBREMPSISTEMAFINANCIERO NOT IN ('BANCO DE CREDITO DEL PER ')
...
BASE_SBS AS (
  ...
  SUM(CASE WHEN FLG_DENEG_REINGRESO_1M_NO_VENTA = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_DENEG_REING_NO_VTA_CON_AUM_SBS_REGLA, --BASE DE DENEGADAS CON REINGRESO Y SIN VENTAS
  SUM(CASE WHEN FLG_DENEG_NOREINGRESO_NO_VENTA = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_DENEG_NOREING_NO_VTA_CON_AUM_SBS_REGLA, -- BASE DE DENEGADAS SIN REINGRESO Y SIN VENTA
  SUM(CASE WHEN FLG_NODESEMBOLSADO = 1 AND FLG_AUMENTO_DEUDA_SBS_REGLA = 1 THEN 1 ELSE 0 END) AS N_APROB_NO_DESEM_CON_AUM_SBS_REGLA -- BASE DE APROBADAS SIN DESEMBOLSO
  FROM BASE_SBS 
