...
MAP_OPORT AS (
  SELECT * FROM LKP_CONSUMO
  UNION ALL
  SELECT * FROM LKP_TARJETA
),
MAP_OPORT_DEDUP AS (
  SELECT CODSOLICITUD, CODINTERNOCOMPUTACIONAL
  FROM (
    SELECT
      CODSOLICITUD,
      CODINTERNOCOMPUTACIONAL,
      ROW_NUMBER() OVER (PARTITION BY CODSOLICITUD ORDER BY PRIO) AS rn
    FROM MAP_OPORT
  ) t
  WHERE rn = 1
),
BASE AS (
  SELECT
      ...
      COALESCE(NULLIF(TRIM(A.CODINTERNOCOMPUTACIONAL), ''), M.CODINTERNOCOMPUTACIONAL) AS CODINTERNOCOMPUTACIONAL_FILLED
  FROM CATALOG_LHCL_PROD_BCP.BCP_EDV_RBMBDN.T72496_BASE_CENTRALIZADO A
  LEFT JOIN MAP_OPORT_DEDUP M
    ON UPPER(TRIM(A.OPORTUNIDAD)) = UPPER(TRIM(M.CODSOLICITUD))
  WHERE A.TIPPRODUCTO = 'CC'
),
BASE_ENRIQ AS (
  SELECT
      A.OPORTUNIDAD,
